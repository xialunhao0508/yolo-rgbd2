...
def check_det_dataset(dataset, autodownload=True):
    ...
    path2 = Path(extract_dir or data.get('path2') or Path(data.get('yaml_file', '')).parent)  # dataset root
    ...
    data['path2'] = path2  # download scripts
    for k in 'train2', 'val2', 'test2':
        if data.get(k):  # prepend path
            if isinstance(data[k], str):
                x = (path2 / data[k]).resolve()
                if not x.exists() and data[k].startswith('../'):
                    x = (path2 / data[k][3:]).resolve()
                data[k] = str(x)
            else:
                data[k] = [str((path2 / x).resolve()) for x in data[k]]

    # Parse YAML
    train, val, test, s = (data.get(x) for x in ('train', 'val', 'test', 'download'))
    train2, val2, test2, s2 = (data.get(x) for x in ('train2', 'val2', 'test2', 'download2'))
